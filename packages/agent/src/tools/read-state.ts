...
export const createReadStateTool = (session: AuditSession): AgentToolDef =>
    ({
        name: 'read_state',
        description:
        ...
        inputSchema: z.object({
            detailUrl: z.string().optional().describe('If provided, return detailed scan results for this specific URL'),
        }),
        run: async ({ detailUrl }: any) => {
            if (detailUrl) {
                const results = session.scanResults[detailUrl];
                if (!results) return `No scan results found for ${detailUrl}. Scanned URLs: ${session.scannedUrls.join(', ') || 'none'}`;
...
function formatSessionOverview(session: AuditSession): string {
    ...
    if (session.scannedUrls.length > 0) {
        ...
        for (const url of session.scannedUrls) {
            ...
            if (r) lines.push(`- ${url}: ${r.summary.totalViolations} violations`);
        }
    }

    if (session.findings.length > 0) {
        const byConfidence = { confirmed: 0, corroborated: 0, 'ai-only': 0, contradicted: 0 };
        for (const f of session.findings) byConfidence[f.confidence]++;
